...
//			"<(([a-zA-Z0-9]+)[:]){0,1}explain[^>]+(xmlns(:\\2){0,1}=\"(.+?)\")[^>]*>");
	 ...
	 */
	public ExplainRecord(RecordType axisRecord)
			throws InternalClientException {
		super(axisRecord);
	}
	...
	protected ExplainData decodeFragmentXML() {
		if(this.resultData != null)
			...
			try {
		    	...
		    	removePrefix(getRecordDataDOM(), 
		    			getRecordDataDOM().getPrefix());
	...
	private void removePrefix(Element element, String prefix) {
    	if(element == null || prefix == null || prefix.length()==0) return;
	 ...
	 * Marshaller here.
	 * 
	 * When putting up an explain request, we get the same explain
	 * response for search and filter requests, containing the same
	 * recordData of the explain-tag BUT using different namespaces.
	 * 
	 * JiBX cannot handle binding definitions of the same mapping name
	 * for the same class using different namespaces. Therefore
	 * set the UnmashallingContext to ignore the namespaces.
	 * The option of disabling namespaces should be considered experimental 
	 * and may not be supported in the future by JiBX.  
	 * 
	 * @param xml
	 * @return
	...
	private ExplainData decodeFragment(String xml) throws JiBXException {
    	...
    	IBindingFactory bfact = BindingDirectory.getFactory(
    			TransportProtocol.REST.name(), ExplainData.class);
	...
	/**
	 * If mapping of recordData or recordDataText fails, do not return an
	 * empty ExplainData instance. Return null instead to imply, that the 
	 * content may be usable by accessing the content of recordData or 
	 * recordDataString directly and evaluating this on your own.
	 */
	@Override
