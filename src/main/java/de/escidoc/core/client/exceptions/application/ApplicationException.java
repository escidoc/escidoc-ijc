...
import java.lang.reflect.Field; public abstract class ApplicationException  extends de.escidoc.core.client.exceptions.EscidocException  implements java.io.Serializable { public ApplicationException(String message, Throwable cause) {super(message, cause);try {Class te = ApplicationException.class;Class cE =Class.forName(te.getName().replace("de.escidoc.core.client.exceptions","de.escidoc.core.common.exceptions.remote"));Field[] tF = te.getDeclaredFields();Field[] cF = cE.getDeclaredFields();for (int i = 0; i < tF.length; ++i) {tF[i].setAccessible(true);cF[i].setAccessible(true);tF[i].set(this, cF[i].get(cause));}} catch (Exception e) { throw new RuntimeException(e);}}
