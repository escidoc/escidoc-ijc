...
import java.lang.reflect.Field; public class FileNotFoundException  extends de.escidoc.core.client.exceptions.application.notfound.ResourceNotFoundException  implements java.io.Serializable { public FileNotFoundException(String message, Throwable cause) {super(message, cause);try {Class te = FileNotFoundException.class;Class cE =Class.forName(te.getName().replace("de.escidoc.core.client.exceptions","de.escidoc.core.common.exceptions.remote"));Field[] tF = te.getDeclaredFields();Field[] cF = cE.getDeclaredFields();for (int i = 0; i < tF.length; ++i) {tF[i].setAccessible(true);cF[i].setAccessible(true);tF[i].set(this, cF[i].get(cause));}} catch (Exception e) { throw new RuntimeException(e);}}
