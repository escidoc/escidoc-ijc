...
public class AggregationDefinitionMarshallerTest
    extends MarshallerTestBase<AggregationDefinition> {
    ...
    public AggregationDefinitionMarshallerTest(final TransportProtocol transport)
        ...
        super(AggregationDefinition.class, BASE, XSD,
            "aggregation-definition_complete.xml", transport);
    }
    ...
    protected void validate(final AggregationDefinition obj) throws Exception {
        ...
        assertXPath("/agg-def:aggregation-definition/agg-def:name",
            obj.getName());
        assertResource("/agg-def:aggregation-definition/agg-def:scope",
            obj.getScope());

        String path =
            "/agg-def:aggregation-definition/agg-def:aggregation-table[1]";
        assertNotNull(obj.getAggregationTables());
        assertNotNull(obj.getAggregationTables().get(0));
        assertXPath(path + "/agg-def:name", obj
            .getAggregationTables().get(0).getName());
        ...
        assertEnum(fieldPath + "/agg-def:type",
            ((InfoField) field).getInfoFieldType());
        assertXPath(fieldPath + "/agg-def:xpath",
            ((InfoField) field).getXPath());
        ...
        assertXPath(fieldPath + "/@feed",
            ((TimeReductionField) field).getFeed());
        assertXPath(fieldPath + "/agg-def:name",
            ((TimeReductionField) field).getName());
        assertEnum(fieldPath + "/agg-def:reduce-to",
            ((TimeReductionField) field).getReduceTo());

        // TODO complete
    }

    @Override
    protected void testSubResources(final AggregationDefinition obj)
        throws Exception {
    ...
    protected void testResourceWithoutSubResources(
        final AggregationDefinition obj) throws Exception {
        // TODO Auto-generated method stub
    }
}