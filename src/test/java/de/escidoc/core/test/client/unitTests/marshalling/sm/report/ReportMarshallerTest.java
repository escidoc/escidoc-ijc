...
public class ReportMarshallerTest extends MarshallerTestBase<Report> {
     ...
     * @throws SAXException
     */
    public ReportMarshallerTest(final TransportProtocol transport)
    ...
    protected void validate(final Report obj) throws Exception {
        assertResource("/report:report/report:report-definition",
        ...
        assertTrue(obj
            .getReportRecord().getParameters().get(0).getParameterType()
            .equals(ParameterType.DATE));
        assertXPath(path + "[1]/@name", obj
            .getReportRecord().getParameters().get(0).getName());
        assertDateTime(path + "[1]/report:datevalue", ((DateParameter) obj
            .getReportRecord().getParameters().get(0)).getValue());

        assertNotNull(obj.getReportRecord().getParameters().get(1));
        assertTrue(obj
            .getReportRecord().getParameters().get(1).getParameterType()
            .equals(ParameterType.STRING));
        assertXPath(path + "[2]/@name", obj
            .getReportRecord().getParameters().get(1).getName());
        assertXPath(path + "[2]/report:stringvalue", obj
            .getReportRecord().getParameters().get(1).getValue());

        assertNotNull(obj.getReportRecord().getParameters().get(2));
        assertTrue(obj
            .getReportRecord().getParameters().get(2).getParameterType()
            .equals(ParameterType.DECIMAL));
        assertXPath(path + "[3]/@name", obj
            .getReportRecord().getParameters().get(2).getName());
        assertXPath(path + "[3]/report:decimalvalue", obj
            .getReportRecord().getParameters().get(2).getValue());
    }
    ...
    @Override
    protected void testResourceWithoutSubResources(final Report obj)
        throws Exception {
        // TODO Auto-generated method stub
    }
}