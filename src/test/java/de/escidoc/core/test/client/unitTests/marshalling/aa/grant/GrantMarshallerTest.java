...
public class GrantMarshallerTest extends MarshallerTestBase<Grant> {
    ...
    protected void validate(final Grant obj) throws Exception {
        ...
        assertXPath(path + "/srel:granted-to/@resource", obj.getProperties().getGrantedTo().getResource());
        ...
        assertDateTime(path + "/prop:revocation-date", obj.getProperties().getRevocationDate());
        assertResource(path + "/srel:revoked-by", obj.getProperties().getRevokedBy());
        assertXPath(path + "/prop:grant-remark", obj.getProperties().getGrantRemark());
        assertXPath(path + "/prop:revocation-remark", obj.getProperties().getRevocationRemark());
        assertResource(path + "/srel:role", obj.getProperties().getRole());
        // xlink cannot be generated in case of SOAP
        assertResource(path + "/srel:assigned-on", obj.getProperties().getAssignedOn(), false);
    }
    ...
    @Override
    protected void testResourceWithoutSubResources(final Grant obj) throws Exception {
        // TODO Auto-generated method stub
    }
}