...
public class ItemHandlerClientTest extends EscidocClientTestBase {
    ...
    public void testFilterBinding() throws Exception {
        String param =
            "<param>"
                ...
                + "<id>escidoc:7048</id>"
                + "</filter>"
                + "<filter name=\"http://escidoc.de/core/01/properties/public-status\">pending</filter>"
                + "<filter name=\"http://escidoc.de/core/01/structural-relations/created-by\">escidoc:user42</filter>"
                + "</param>";
    ...
    public void testCreateAndSubmit() throws Exception {
        ...
        ItemHandlerClient ic = new ItemHandlerClient();
        ic.setServiceAddress("http://localhost:8080");
        Item item = ic.retrieve(EXAMPLE_ITEM_ID);
