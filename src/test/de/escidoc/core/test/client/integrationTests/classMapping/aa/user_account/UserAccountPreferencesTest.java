...
public class UserAccountPreferencesTest {
    ...
    public void testPreferences01() throws Exception {
        ...
        assertEquals("Preference name differs", uaPref.getName(), createPref
            .getName());
        assertEquals("Preference value differs", uaPref.getValue(), createPref
            .getValue());

        // retrieve
        Preferences preferences = uahc.retrievePreferences(objId);
        assertTrue("Wrong number of preferences", preferences
            .getPreferences().size() == 1);
        Preference p = preferences.getPreferences().iterator().next();
        assertEquals("Preference name differs", uaPref.getName(), p.getName());
        assertEquals("Preference value differs", uaPref.getValue(), p
            .getValue());
    }
