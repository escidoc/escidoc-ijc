package de.escidoc.core.test.client.aa.actions;
...
public class ActionHandlerClientTest extends EscidocClientTestBase {

    private final Logger logger =
        Logger.getLogger(ActionHandlerClientTest.class.getName());
    ...
    public void testCreateAndRetrieveSuccessfulActions() throws Exception {
        ...
        ac.setHandle(EscidocClientTestBase.DEFAULT_HANDLE);
        //rc.setServiceAddress("http://localhost:8080");
        UnsecuredActions actions = new UnsecuredActions();
        actions.addAction("1");
        actions.addAction("2");
        
//        Marshaller<UnsecuredActions> m = new Marshaller<UnsecuredActions>(actions.getClass());
...
//        System.out.println("actions " + actionsXml);
        UnsecuredActions ua = ac.createUnsecuredActions("escidoc:persistent3", actions);
        String retrievedActionsXml =
            Factory.getUnsecuredActionsMarshaller().marshalDocument(
                (UnsecuredActions) ac.retrieveUnsecuredActions("escidoc:persistent3"));
